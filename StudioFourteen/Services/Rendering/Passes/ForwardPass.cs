...
public class ForwardPass : InstanceRenderPassBase<ForwardPass.ForwardPassData>
{
	...
	{
		Matrix4x4 viewProj = renderer.GetViewMatrix(renderer) * renderer.GetProjectionMatrix(renderer);
		...
		lock (this.sceneObjects)
		{
			foreach (DrawObject draw in this.sceneObjects)
			{
				...
				catch (Exception ex)
				{
					...
					Studio.Log.Error(ex, $"Error hit testing forward object: {draw}. This object will be disabled.");
				}
			}
		}
	}
	...
	public override unsafe void Render(Renderer renderer, Device device, DeviceContext deviceContext)
	{
		...
		this.PassData.ViewMatrix = Matrix4x4.Transpose(renderer.GetViewMatrix(renderer));
		this.PassData.ProjectionMatrix = Matrix4x4.Transpose(renderer.GetProjectionMatrix(renderer));
		this.PassData.CameraPosition = new Vector4(renderer.GetCameraPosition(renderer), 1);
