...
public class RaceLibraryEntry : ExcelLibraryEntry
{
		...
		: base(source, race.RowId)
	{
		...
		if (this.RowId > 0)
		{
			ExcelSheetLibrarySource<TribeLibraryEntry>? tribeSource = Studio.Library.GameData.GetLibrarySource<TribeLibraryEntry>();
			if (tribeSource != null)
			{
			...
			this.Genders.Add(StudioFourteen.Services.Library.GameData.Genders.Masculine);
			this.Genders.Add(StudioFourteen.Services.Library.GameData.Genders.Feminine);
		}
	}
