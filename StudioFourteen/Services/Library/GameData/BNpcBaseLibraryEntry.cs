...
using BNpcCustomize = StudioFourteen.Services.Library.GameData.Sheets.BNpcCustomize;

public class BNpcBaseLibraryEntry : ExcelLibraryEntry
{
		...
		: base(source, npc.RowId)
	{
		...
		if (Studio.Library.GameData.BattleNpcNameIndex.TryGetValue($"{this.bNpcBase.RowId}", out uint nameRowId))
		{
			BNpcName? name = Studio.DataManager.GetRow<BNpcName>(nameRowId);
			if (name != null)
			{
	...
	public CustomizeData? Customize
	{
		get
		{
			BNpcCustomize? customize = Studio.DataManager.GetRow<BNpcCustomize>(this.bNpcBase.BNpcCustomize.RowId);
...
public class BNpcBaseLibrarySource : ExcelSheetLibrarySource<BNpcBase, BNpcBaseLibraryEntry>
{
	...
	private void Deduplicate()
	{
		...
		ExcelSheet<BNpcBase>? battleNpcSheet = Studio.DataManager.GetExcelSheet<BNpcBase>();
	...
	private void Deduplicate(ExcelSheet<BNpcBase> sheet, ref int count)
	{
		...
		{
			string? name = null;
			if (Studio.Library.GameData.BattleNpcNameIndex.TryGetValue($"{npc.RowId}", out uint nameRowId))
			{
				BNpcName? bNpcName = Studio.DataManager.GetRow<BNpcName>(nameRowId);
				if (bNpcName != null)
				{
