...
public class KeyboardDevice : InputDeviceBase
{
	...
	public KeyboardDevice()
	{
		...
		foreach (VirtualKey key in Studio.KeyState.GetValidVirtualKeys())
		{
	...
	{
		if (Studio.IsDisposed || !Studio.IsInitialized || Studio.Input.IsXivTextInputActive)
			return;
	...
	public IEnumerable<VirtualKey> GetValidKeys()
	{
		...
		return Studio.KeyState.GetValidVirtualKeys();
	}
	...
	public bool HandleKey(VirtualKey vKey, bool down)
	{
		...
		if (!Studio.KeyState.IsVirtualKeyValid(vKey))
			return false;
